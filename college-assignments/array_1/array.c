...
array *new_merge(array *a1, array *a2) {
    if((a1->size - a1->len) >= (a2 -> len)) {
        for(int i = a1 -> len; i < a1->size;i++) {
            a1->arr[i] = a2 -> arr[i - a1 -> len];
        }
        free(a2);
    }else {
        a1 -> arr = (int *)realloc(a1->arr, sizeof(int) * (a1->len + a2->len + 1));
